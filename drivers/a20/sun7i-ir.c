...
static struct timer_list *s_timer_led; 
static int led_flash_used = 0;
static struct gpio_config gpio_led_flash_on, gpio_led_flash_off;

static void led_timer_handle(unsigned long arg)
{
    del_timer(s_timer_led);
    sw_gpio_setall_range(&gpio_led_flash_on, 1);
}

static inline void input_report_key_mod(struct input_dev *dev, unsigned int code, int value)
{
    if (led_flash_used && value) {
         sw_gpio_setall_range(&gpio_led_flash_off, 1);
         if (!timer_pending(s_timer_led)) {
             s_timer_led->expires = jiffies + (HZ/5);  //200ms timeout
             add_timer(s_timer_led);
         } /* else
             mod_timer(s_timer_led, jiffies + (HZ/5)); */
    }
    // printk("rcode:%02x\n", code);
...
static inline const unsigned int * get_ir_keycodes_by_customer_code(unsigned long code)
{
    // constomer code: black 0x50af/0xbd02, white fe01, common ff00
    int constomer_code = code & 0x00ff;
    if (constomer_code == 0x50) 
        constomer_code = 0x02;
    else if (constomer_code > MAX_CONSTOMER_NUM) 
        constomer_code = 0;
    return ir_keycodes_contomers[constomer_code];
}
	...
	if (intsta & IR_RXINTS_RXPE) {	 /* Packet End */
		...
		if(timer_used) {
			if(code_valid) {  //the pre-key is released
				...
				input_report_key_mod(ir_dev, get_ir_keycodes_by_customer_code(ir_code)[(ir_code>>16)&0xff], 0);
		...
		if(timer_used) {
			...
			if(ir_cnt == 1) {
				...
				input_report_key_mod(ir_dev, get_ir_keycodes_by_customer_code(ir_code)[(ir_code>>16)&0xff], 1);
...
static void init_keycodes_map(unsigned int map[], unsigned int const src[][2], int size)
{
    int i;
    for (i=0; i<size; i++) {
        map[src[i][0]] = src[i][1];
...
static void init_keycodes_maps(void)
{
    init_keycodes_map(ir_keycodes_common, ir_keycodes_map_common, sizeof(ir_keycodes_map_common)/sizeof(int[2]));
    init_keycodes_map(ir_keycodes_white, ir_keycodes_map_white, sizeof(ir_keycodes_map_white)/sizeof(int[2]));
    init_keycodes_map(ir_keycodes_black, ir_keycodes_map_black, sizeof(ir_keycodes_map_black)/sizeof(int[2]));
}

static void init_led_flash(void)
{
    int used = 0;
    script_item_u   val;
    if ((script_get_item("gpio_led_flash", "led_flash_used", &val) == SCIRPT_ITEM_VALUE_TYPE_INT)
            && (val.val == 1)) {
        used = 1;
    }
    led_flash_used = used;
    if (used) {
        if (script_get_item("gpio_led_flash", "led_on", &val) == SCIRPT_ITEM_VALUE_TYPE_PIO) {
            memcpy(&gpio_led_flash_on, &val.gpio, sizeof(gpio_led_flash_on));
        }
        if (script_get_item("gpio_led_flash", "led_off", &val) == SCIRPT_ITEM_VALUE_TYPE_PIO) {
            memcpy(&gpio_led_flash_off, &val.gpio, sizeof(gpio_led_flash_off));
        }
    }

    if (led_flash_used) {
        s_timer_led = kmalloc(sizeof(struct timer_list), GFP_KERNEL);
        if(!s_timer_led) {
            printk(" IR FAIL TO  Request Time\n");
            led_flash_used = 0;
        }
        else {
            init_timer(s_timer_led);
            s_timer_led->function = &led_timer_handle;
        }
    }
}
